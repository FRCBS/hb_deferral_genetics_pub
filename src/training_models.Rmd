...
### 1.3.5. Split regression and cox data to train, test and validate data sets (inc. prevalence tables)
...
#### Common donors in regression and cox data sets
...
### 1.3.6. Data for cox with time-dependent covariates
...
## 1.4. Figures to examine (train) data sets

### 1.4.1 Table 1. for the regression and cox
...
### 1.4.3.Correlation matrices for regression_train data
...
## 1.5. Deferrals by years in the different whole data sets
...
## 1.6. Total genetic risk from train data
...
# is the reason for too good AUC and AUPR values
...
### Fit models
```{r Fitting freq model for the whole data}
...
```{r Correlations tsv and pdf}

d <- descript %>% mutate(Pretty = case_when(Pretty == "Days to previous full blood donation" ~ "Days to previous fbd*",
...
Krista: Jarkko had made his own functions and I used them first, but he has since witched to broom::tidy(fit, conf.int=TRUE, exponentiate = TRUE).
...
### 3.3.1 Define model parameters and formulas for Bayesian logistic models

```{r Parameters for Bayesian models}
iterations <- 1000
cores <- 4   # Jarkko: Run out of memory with four cores
...
### 3.3.4. Effect distribution

```{r Forest plot Bayesian models}
...
### 3.3.5 Check linearity - Splines (FULL SPLINE MODEL)
...
full_spline_datas <- list(#all=data_all,
                          male=data_male, female=data_female)
                          #pre_menopausal_female=data_pre_menopausal_female,
                          #post_menopausal_female=data_post_menopausal_female)

full_spline_formulas <- list(#all=full_spline_formula,
...
Jarkko: This seems to run out-of-memory when I tried to rerun it! - for spline model for whole data.
...
## 3.5. Bayesian models with splines for some variables 

### 3.4.1. Define variables and formulas for spline models
...
### 3.4.3. Plot unscaled and scaled spline models
...
filename <- sprintf("%s/tdc_fits_train.rds", base)
