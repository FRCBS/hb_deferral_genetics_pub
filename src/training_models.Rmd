...
#recompute_cox <- TRUE
cox_filename <- sprintf("%s/unfiltered_data_cox.rds", data_path)
if (recompute_datas | !file.exists(cox_filename)) {
  old_n <- length(unique(preprocessed_donations_recoded_snps$donor_id))
  
  unfiltered_data_cox  <- preprocessed_donations_recoded_snps %>%
  ...
  new_n <- length(unique(unfiltered_data_cox $donor_id))
  
  cat("Dropping", {old_n - new_n}, "donors due to filtering first events out. \n\n")
  
  saveRDS(unfiltered_data_cox , cox_filename)
} else {
  
  # Load existing unfiltered_data_cox 
  unfiltered_data_cox  <- readRDS(cox_filename) %>% ungroup()
  cat("Loaded precomputed unfiltered_data_cox .")
}
```
...
filename <- sprintf("%s/cox_data.rds", data_path)
if (recompute_datas | !file.exists(filename)) {
  ...
  data_cox <- preprocessed_donations_recoded_snps %>% 
    ...
    inner_join(unfiltered_data_cox , by=c("donor_id", "donation_date")) %>%
  ...
  filename2 <- sprintf("%s/exclusions_deferral_cox.tsv", data_path)
...
if(recompute_datas | !file.exists(filename)) {
  ...
  filename2 <- sprintf("%s/exclusions_tdc.tsv", data_path)
...
### 3.3.1. Fit models and fit summaries
...
if (recompute | !file.exists(filename)) {
  freq_int_cis <- map_dfr(freq_int_fits_list, freq_int_cis_helper, .id = "group")
  write_tsv(freq_int_cis, file=filename)
} else {
  freq_int_cis <- read_tsv(filename)
  cat("Loaded precomputed freq_int_cis")
}
...
```{r}
# Chenking
#table(freq_int_cis$group, useNA = "ifany")
```
...
```{r Test - Prettier and clearer trace plots - spline, eval=FALSE}
# UNCOMMEND and modify if clearer plots are needed
# helper <- function(group, fit) {
#   # Convert fit to array and include warmup
#   draws_array <- as_draws_array(fit, inc_warmup = TRUE)
#   n_col <- ncol(as_draws_df(draws_array))
...
```{r Final - Prettier and clearer trace plots - spline, eval=FALSE}
# UNCOMMEND and modify if clearer plots are needed
# helper <- function(group, fit) {
#   # Convert fit to array and include warmup
#   draws_array <- as_draws_array(fit, inc_warmup = TRUE)
...
## 3.7. Combine confidence intervals from the three methods on full data (NOT needed for this project!)
...
## 3.8. Odds ratios of high genetic risk versus low genetic risk (NOT needed for this project!)
...
# Function to help scale the cox data
...
```{r Checking the scaled cox data for males}
tmp <- scaled_sur_male %>% 
...
```{r KM cox_train survival from HB deferral and survival propability}
...
# Male
# SET smooth = TRUE for smoothed curves
...
```{r Plotting cox effect sizes, fig.dim=c(8,12)}
...
```{r Extract PH test p values - cox}
...
```{r Table for the PH p-values - cox}
#library(webshot2)
...
![PH test p-values cox](../training_models/training_with_snp1_group/figures/ph_test_results_cox.png)

#### Schoenfeld residuals

```{r Bad schoenfeld residual plots, eval=FALSE}
...
Jarkko:
It’s also possible to check outliers by visualizing the deviance residuals. The deviance 
...
# Function to help to scale the tdc data set
scale_helper <- function(df) {
...
# Combined all the datas in one list
...
```{r Extract PH test p values - tdc}
res <- map_dfr(tdc_fits_list, extract_ph_stats, .id="model") %>% 
  pivot_wider(names_from = model, values_from=p)

# THIS pvalue_table() function form common.R did not work!
#pvalue_table(res)
#print(res)
print.data.frame(res)
```

```{r Table for the PH p-values - tdc}
custom_color <- function(x) {
  ifelse(x < 0.05, "#D55E00", "#0072B2")
}
...
![PH test p-values tdc](../training_models/training_with_snp1_group/figures/ph_test_results_tdc.png)
...
filename <- sprintf("%s/tdc_cis.tsv", table_path)
if (recompute | !file.exists(filename)) {
...
if (save_figs) {
  filename <- sprintf("%s/tdc_cis.png", fig_path)
  ggsave(filename=filename,  plot=g, width = 180,  height = 300, units="mm", dpi=300, scale=1.0, device="png")
}
g
```
...
## 4.6. Bayesian survival (NOT INCLUDED INTO THIS PROJECT!)
### 4.6.1. brms way
...
## 5.1. Load cis if needend

```{r Loading cis lists}
# UNCOMMENT if only plotting done
# freq_cis_groups <- read_tsv(sprintf("%s/freq_groups_cis.tsv", table_path))
# freq_int_cis <- read_tsv(sprintf("%s/freq_int_groups_cis.tsv", table_path))
...
# tdc_cis <- read_tsv(sprintf("%s/tdc_cis.tsv", table_path))
```

## 5.2. Preparation for the foret plots (names, labels, colors etc.)
...
## 5.3. Combine cis to one table

```{r}
# Add column model to all the data frames
freq_cis_groups$model <- "LogR"
freq_int_cis$model <- "LogRInt"
bayes_cis$model <- "BayesLogR"
spline_cis$model <- "BayesLogRSpline"
cox_cis$model <- "Cox"
tdc_cis$model <- "Tdc"

# Combine the cis tables from all the models
# Renaming the Prettys for the interactions
combined_cis <-
  bind_rows(freq_cis_groups, spline_cis, freq_int_cis, cox_cis,
            bayes_cis, tdc_cis) %>%
...
## 5.4. Plotting for separate forest plots for groups
...
# Create the forest plot for group all
g <- groupped_tables$all %>% 
  mutate(Pretty = fct_rev(Pretty)) %>%
  mutate(Model = factor(model, levels = mymodels)) %>% #set the order of the models as wanted
...
# Create the forest plot for group male
...
# Create the forest plot for group female
...
## 5.5. Separate plots for non genomic and genomic variables
...
```{r Combined effect sizes: non-genomic variables}

# Combined effect sizes: non-genomic variables
g <- combined_cis %>%
  ...
  geom_stripes(freq_int_cis %>% filter(term %in% non_genomic_variables), Pretty) +
  ggstance::geom_linerangeh(position=position_dodge(width=1), 
                            size=0.2
                            #linewidth=0.2
                            ) + 
...
# Combined effect sizes: genomic variables 1
g <- combined_cis %>%
  ...
  geom_stripes(freq_int_cis %>% filter(term %in% genomic_variables_1), Pretty) +
  ggstance::geom_linerangeh(position=position_dodge(width=1), 
                            size=0.2
                            #linewidth=0.2
                            ) + 
...
# Combined effect sizes: genomic variables 2
g <- combined_cis %>%
  ...
  geom_stripes(freq_int_cis %>% filter(term %in% genomic_variables_2), Pretty) +
  ggstance::geom_linerangeh(position=position_dodge(width=1), 
                            size=0.2
                            #linewidth=0.2
                            ) + 
